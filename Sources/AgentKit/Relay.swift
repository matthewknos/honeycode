...
enum Relay {
    ...
    private static func artifact(in reply: String) -> String? {
        let found = renderableBlocks?(reply) ?? []
        // A reply carrying two artifacts stays inline — see above.
        guard found.count == 1 else { return nil }
        // Long enough to be a page rather than a snippet, and most of the reply.
        guard found[0].count >= 400, found[0].count * 2 >= reply.count
        else { return nil }
        return found[0]
    }

    /// The renderable documents in a reply, in order, as the host's own
    /// markdown parser sees them.
    ///
    /// This used to call `MarkdownText.blocks` directly, which is a 340-line
    /// block parser nested inside a SwiftUI view — the one piece of the
    /// transcript that hasn't come across yet. Reimplementing a fence scanner
    /// here instead would be worse than a seam: the doc comment above turns on
    /// SVG never reaching this shape *because `MarkdownText` rewrites it first*,
    /// so a second parser that disagreed by a fence would change what travels
    /// as a file, silently.
    ///
    /// The policy stays here — one block, long enough, most of the reply. The
    /// host supplies only the parse it uniquely has. Unset means replies always
    /// travel as text, which is the correct answer for a host with no renderer.
    nonisolated(unsafe) static var renderableBlocks: ((String) -> [String])?
    ...
    static func destination(_ typed: String, in workspace: Workspace,
        ...
        let scored = candidates.compactMap { session -> (Session, Int)? in
            ...
            let name = Fuzzy.score(typed, in: session.name)
            let full = Fuzzy.score(typed, in: "\(session.account.shortTitle) \(session.name)")
