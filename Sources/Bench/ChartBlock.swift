...
struct ChartSpec {
    ...
    func values(at x: String) -> [(name: String, value: Double)] {
        series.compactMap { series in
            series.points.first { $0.x == x }.map { (series.name, $0.y) }
        }
    }
    ...
    private func readout(_ category: String) -> some View {
        VStack(alignment: .leading, spacing: 2) {
            Text(category)
                .font(.system(size: 10, weight: .medium))
                .foregroundStyle(.secondary)
            ForEach(Array(spec.values(at: category).enumerated()), id: \.offset) { index, entry in
                HStack(spacing: Theme.s3 - 2) {
                    if !spec.isSingleSeries {
                        Circle()
                            .fill(ChartPalette.colour(at: index))
