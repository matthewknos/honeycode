...
final class DeltaCoalescer {
    ...
    private func step() {
        lock.lock()
        let backlog = runs.reduce(0) { $0 + $1.text.count }
...
final class ClaudeAdapter: AgentAdapter {
    ...
    static var skipPermissions: Bool {
        UserDefaults.standard.object(forKey: "agent.skipPermissions") as? Bool ?? true
    }

    /// Posted by Settings when the permission toggle changes.
    static let permissionsChanged = Notification.Name("bench.permissionsChanged")
    ...
    private func startIfNeeded() -> Bool {
        ...
        arguments += ["--append-system-prompt", Self.hostCapabilities]
    ...
    func send(_ text: String) {
        ...
        guard var data = try? JSONSerialization.data(withJSONObject: turn) else { return }
    ...
    func interrupt() {
        guard started else { return }
        // Closing stdin ends the run cleanly. A finer-grained mid-turn interrupt
        // needs the control protocol, which isn't wired up here.
        try? inPipe.fileHandleForWriting.close()
        process.terminate()
        started = false
    }
    ...
    private func route(_ line: Data) {
        ...
        case "user":
            ...
            for block in content where block["type"] as? String == "tool_result" {
                ...
                DispatchQueue.main.async {
                    self.session.resolve(toolUseID: useID, state: state)
                    self.session.noticeDevServer(in: text)
                }
            }
    ...
    private static func diff(tool: String, input: [String: Any]) -> (String, [DiffRow])? {
        ...
        case "Edit":
            ...
            guard !current.isEmpty, current.contains(old) else {
                return (String(label), Diff.rows(from: old, to: new))
            }
            return (String(label),
                    Diff.rows(from: current, to: current.replacingOccurrences(of: old, with: new)))
