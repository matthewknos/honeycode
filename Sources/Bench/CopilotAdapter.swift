...
///
/// One gotcha: the default model can be `kimi-k2.7-code`, which rejects the
/// reasoning effort the CLI sends and fails the very first turn with a bare
/// error string. `session/set_model` is not optional.
final class CopilotAdapter: AgentAdapter {
    ...
    private func startIfNeeded() -> Bool {
        ...
        guard let binary = Self.locateCopilot() else {
            emit(.notice(id: UUID(), text: "Couldn't find the `copilot` binary on PATH."))
            return false
        }
            ...
            DispatchQueue.main.async {
                ...
                if proc.terminationStatus != 0 {
                    self.session.items.append(.notice(id: UUID(), text:
                        text.isEmpty ? "copilot exited unexpectedly." : text))
                }
            }
        }
        ...
        do {
            ...
            emit(.notice(id: UUID(), text: "Couldn't start copilot: \(error.localizedDescription)"))
            return false
        }
    ...
    private static func locateCopilot() -> URL? {
        let candidates = [
    ...
    private func handle(replyID id: Int, json: [String: Any]) {
        if let error = json["error"] as? [String: Any] {
            ...
            let message = error["message"] as? String ?? "Copilot returned an error."
            DispatchQueue.main.async { self.session.isRunning = false }
    ...
    private func finish(_ kind: Probe) {
        ...
        case .context:
            // Empty until the agent has built its context — the reply on a
            // fresh session is "not yet available", which simply won't parse.
            guard let usage = Self.context(in: text) else { return }
            DispatchQueue.main.async {
                self.session.context = usage
                // Every turn re-sends the whole window, so summing what was in
                // it at each turn's end is the closest thing to cumulative
                // input that Copilot will tell us. It's a floor, not a meter:
                // a turn that runs six tools sends the context six times, and
                // the CLI rounds to the nearest 0.1k before we ever see it.
    ...
    private func adoptModels(from result: [String: Any]) {
        ...
        let parsed = ModelCatalog.copilotModels(list)
    ...
    private func handle(update: [String: Any]) {
        ...
        case "tool_call_update":
            ...
            let output = update["rawOutput"] as? [String: Any]
            let message = output?["content"] as? String ?? ""
            ...
            DispatchQueue.main.async {
                self.session.resolve(toolUseID: callID, state: state)
                self.session.noticeDevServer(in: message)
            }
        ...
        case "plan":
            ...
            DispatchQueue.main.async { self.session.setPlan(todos) }
    ...
    private func emit(_ item: TranscriptItem) {
        DispatchQueue.main.async { self.session.append(item) }
    }
}
