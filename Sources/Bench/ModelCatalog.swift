...
enum ModelCatalog {
    ...
    static func models(for account: Account) -> [AgentModel] {
        guard let configDir = account.configDir else { return copilotFallback }
        return claudeModels(configDir: configDir)
    }

    // MARK: Claude

    /// Models Bench knows how to present, best first. Entitlement filters this
    ...
    /// set. Reading it is how Bench can tell that a work seat has Opus 4.8 but
    ...
    private static let copilotCacheKey = "copilot.models"
    ...
    static var copilotFallback: [AgentModel] {
        if let data = UserDefaults.standard.data(forKey: copilotCacheKey),
    ...
    static func copilotModels(_ list: [[String: Any]]) -> [AgentModel] {
        ...
        if let data = try? JSONEncoder().encode(models), !models.isEmpty {
            UserDefaults.standard.set(data, forKey: copilotCacheKey)
        }
        return models
    }
