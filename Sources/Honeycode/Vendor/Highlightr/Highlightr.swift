...
{
    /// Returns the current HighlightTheme.
    open var theme : HighlightTheme!
    {
        didSet
        {
            themeChanged?(theme)
    ...
    open var themeChanged : ((HighlightTheme) -> Void)?
    ...
    public init?(highlightPath: String? = nil)
    {
        ...
        // Upstream defaults to "pojoaque" and returns nil when the CSS is
        // missing — and Bench ships two themes, not all 271. Defaulting to one
        // we actually bundle is the difference between highlighting working and
        // the initialiser silently failing.
    ...
    /**
     ...
     - parameter to: HighlightTheme name
    ...
    open func setTheme(to name: String) -> Bool
    {
        ...
        theme =  HighlightTheme(themeString: themeString)
